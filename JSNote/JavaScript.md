...
引用类型就像 Google Sheet —— **所有使用同一个链接（同一个引用）的人，链接指向的都是同一个内存地址,** 看到的都是同一个内容。你改了，别人也会看到变化。
...
![image-20240306090824763](C:\Users\simon.cheng\AppData\Roaming\Typora\typora-user-images\image-20240306090824763.png)
...
![image-20240306090901083](C:\Users\simon.cheng\AppData\Roaming\Typora\typora-user-images\image-20240306090901083.png)
...
在 JavaScript 中，函数作为参数传递时不会立即被调用，而是在被调用的函数内部根据需要执行。当您将一个函数作为参数传递给另一个函数时，实际上是将函数的引用传递给了该函数，而不是执行函数本身。换句话说，函数参数接收到的是一个函数的指针，它指向内存中存储的函数代码，而不是实际的函数执行结果。
...
使用 `new` 关键字调用构造函数时，执行过程如下：

1. **创建新对象**：在内存中创建一个空对象。
2. **绑定 `this`**：将新建的对象绑定到构造函数的 `this` 上，使构造函数中的 `this` 指向这个新对象。这样，在构造函数中定义的属性或方法会添加到这个新对象上。
3. **执行构造函数代码**：逐行执行构造函数中的代码，对 `this` 指向的对象进行初始化操作，比如设置属性或定义方法。
4. **隐式返回**：构造函数默认会返回 `this` 所指向的新对象，因此不需要显式使用 `return` 语句来返回这个对象。

例子：  

在 JavaScript 中，如果使用的是传统的函数构造器, 通常采用大写字母开头的命名方式（比如 `function Person(name, age) {...}`），确实**不需要显式使用 `constructor`** 关键字。任何函数都可以当作构造函数，只要使用 `new` 关键字来调用它。不过，如果你使用的是 **ES6 的类语法**（`class`），则会用到 `constructor` 关键字
...
所有的 JavaScript 对象都会从一个 prototype（原型对象）中继承属性和方法，并提供了一种灵活的继承机制：

- `Date` 对象从 `Date.prototype` 继承。
- `Array` 对象从 `Array.prototype` 继承。
- `Person` 对象从 `Person.prototype` 继承。

- 所有 JavaScript 中的对象都是位于**原型链顶端的 Object 的实例**。使用字面量创建对象时，这个对象的原型会指向 `Object.prototype`
...
```
 例子：
```javascript
 let arr = [123,"hello",true,null];  
```

 **如果要使用字符串键值对，应该使用对象（Object）而不是数组。数组通常用于存储有序的集合，而对象则用于存储键值对。**		  
...
它用于查找数组中满足条件的第一个元素，并返回该元素在数组中的索引。如果没有找到满足条件的元素，则返回 -1。
...
是**数组的一个方法**，用于创建一个**新数组**，新数组的元素是对原数组中的每个元素应用提供的函数后的结果。以下是一个 `map` 的例子：
...
是 JavaScript 最基本的迭代结构，用于遍历数组的每个元素，或者对象的每个属性等。for 循环有多种形式，以下是一些常见的用法，以下是一个简单的 `for` 循环遍历数组的例子：
...
//for...in 循环用于遍历对象的可枚举属性，通常不推荐用于遍历数组
...
方法来遍历数组（不兼容IE8）  ,`forEach` 是数组的一个方法，用于遍历数组的每个元素，并对每个元素执行提供的回调函数。以下是一个 `forEach` 的例子：
...
- 无法使用 `break` 或 `return` 中断循环。
  ...
  - `for` 循环是最基本的迭代结构，可以执行任意代码，需要手动管理索引。
  - `forEach` 是数组的方法，专门用于遍历数组元素，不能中断循环。
  - `map` 是数组的方法，用于创建一个**新数组**，通过对每个元素应用函数来得到新数组。
...
**数据交换格式：** JSON 是一种数据交换格式，用于在不同系统之间传递和存储数据

**字符串表示：** JSON 数据是以字符串的形式存在的，可以通过 `JSON.stringify()` 将对象转换为 JSON 字符串，通过 `JSON.parse()` 将 JSON 字符串转换为对象。
...
**语法限制：** JSON 的语法更为严格，例如，属性名称必须使用双引号括起来。
...
- JavaScript 对象是一种数据结构，可以包含不同类型的值和方法，而 JSON 是一种数据交换格式，用于表示数据的字符串表示。
- JSON 是一个字符串，它具有自己的语法规则，而 JavaScript 对象是 JavaScript 语言的一部分，具有更灵活的语法和功能。
- JSON 只是一种数据表示方式，它可以在不同的编程语言之间共享和解析，而 JavaScript 对象是 JavaScript 语言的核心组成部分。

虽然它们有相似之处，但在语法和使用场景上有一些不同。 JSON 主要用于数据交换，而 JavaScript 对象用于在 JavaScript 程序中组织和操作数据。
...
- 无法访问实例属性和实例方法，因为它们不依赖于特定的实例。

  通常用于定义与类关联的功能，但这些功能不会涉及实例状态。

  适合放置工具函数或逻辑上属于类但不需要实例的行为。
