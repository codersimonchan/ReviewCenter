...
**块级作用域可以**在“**只需要隔离变量作用域**”时**取代 IIFE**；
 ❌ 但如果你需要**立即执行某段逻辑并返回结果**，还是得用 IIFE！**大部分时候我们可以稍后调用，但有些特定场景下，\**我们\**就是想立刻执行一段“临时性的、一次性的逻辑”**，并且让里面的变量**不会泄漏到外部作用域**，这时候 IIFE 是非常合适的。比如初始化逻辑，只想运行一次，或者创建私有变量或 ‘封装作用域’ 等。
...
引用类型就像 Google Sheet —— **所有使用同一个链接（同一个引用）的人，链接指向的都是同一个内存地址,** 看到的都是同一个内容。你改了，原来的变量ye'hui别人也会看到变化。
...
| :-------------------------------- | :----------------------------------------- | :-------------------------------------------------- |
| **Iterates Over**                 | Enumerable properties (keys) of an object. | Values of iterable objects (e.g., arrays, strings). |
| **Works With**                    | Objects (`{}`).                            | Iterables (arrays, strings, maps, sets).            |
| **Includes Inherited Properties** | Yes (from prototype chain).                | No (only iterates over own values).                 |
| **Use Case**                      | Looping over object keys.                  | Looping over values in collections.                 |

for...of==》Looping over values in collections.
